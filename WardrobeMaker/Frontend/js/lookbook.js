    ...
    async loadOutfits() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error('[Lookbook] /lookbook fetch failed', response.status, text);
                throw new Error('Failed to load lookbook');
            }

            const raw = await response.json().catch(() => null);
            console.debug('[Lookbook] raw response:', raw);
            this.outfits = normalizeOutfits(raw);
            console.debug('[Lookbook] normalized outfits:', this.outfits);
            this.render();
        } catch (err) {
        ...
        try {
            grid.innerHTML = this.outfits.map((outfit) => {
                ...
                const imagesHtml = itemSlots.map(({ item, label, cssClass }) => {
                    const imagePath = item?.imageFilePath ?? item?.image ?? item?.imageFile ?? '';
                ...
                const scheduleText = outfit.scheduledDate ?? outfit.ScheduledDate ?? outfit.date ?? outfit.Date ?? '';
                const scheduledHtml = scheduleText
                    ? `<span class="text-[10px] font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-full uppercase tracking-tighter"><i class="fas fa-calendar-alt mr-1"></i> ${scheduleText}</span>`
                    : '';
            ...
            console.error('Error rendering lookbook items:', err);
            grid.innerHTML = '<div class="col-span-full text-center text-sm text-red-400 py-12">Failed to render lookbook items.</div>';
            if (controls) controls.classList.add('hidden');
        }
    },
