...
const WardrobeApp = {
        ...
        if (fileInput.files && fileInput.files[0]) {
            ...
            try {
                ...
                if (!uploadRes.ok) {
                    ...
                    showInlineError('modalError', err.message || 'Upload failed.'); return;
                }
                const uploadData = await uploadRes.json();
                imagePath = uploadData.filePath;
            } catch (err) {
                showInlineError('modalError', 'Failed to upload photo. Please try again.'); return;
            }
        }
    ...
    async toggleLaundry(itemId) {
        try {
            const response = await fetch(`${API_URL}/toggle/${itemId}`, { method: 'POST' });
    ...
    async deleteItem(itemId) {
        ...
        try {
            const response = await fetch(`${API_URL}/inventory/${itemId}`, { method: 'DELETE' });
